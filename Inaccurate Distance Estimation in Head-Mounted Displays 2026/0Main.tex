...
\begin{document}
	...
	\begin{abstract}
        Virtual and augmented reality head-mounted displays (HMDs) render and display head-tracked, stereo images to create an egocentric, 3D percept to the viewer. Errors in stereo geometry introduced by rendering from an incorrect location (e.g., from tracking errors) or viewing from the wrong position (e.g., from fitment errors) can alter the depth and distance a user perceives compared to the intended geometry. A model that can accurately predict distance perception is crucial for characterizing error tolerence and informing headset design. In this work we build an error emulator platform with eye tracking and evaluate the effect of inaccurate stereo geometry on distance perception and compare with a pure geometric ray intersection model. In a reaching experiment, we show that some stereo geometry errors can induce both under- and over-reaching, and the magnitude of error is consistent with geometric predictions. Surprisingly, we find the mismatches between HMD display separation and viewer interpupillary distance error have significantly less impact on reach behavior than predicted by geometry prediction. We then investigate how robust humans are to stereo geometry errors by measuring psychometric performance across a range of error levels. We find a relatively large detection threshold ($\approx$6 mm) to mismatches between viewer interpupillary distance and headset display separation. We additionally show how perceptual thresholds to various stereo geometry errors change with viewing distance and scene complexity.
	\end{abstract}
...
% 	\keywords{virtual reality, distance underestimation, rendering errors, world-locked rendering}	
	...
	\input{2Related}
    \input{3Model} 
	\input{4ReachingExperiment}
	\input{5VisualPerceptionExperiment}   
	\input{6Discussion}
